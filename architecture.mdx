---
title: "How Timeful Is Built: Architecture Overview"
...
Timeful is a classic three-tier web application: a Vue 2 single-page app communicates with a Go REST API, which in turn reads and writes to MongoDB. In production the three tiers run as isolated Docker containers on a private bridge network, with only the Nginx-fronted web port exposed to the host. Understanding this layout is useful whether you are debugging a deployment, contributing a feature, or integrating with the API.
...
└── port 3002 ──► timeful-frontend  (Nginx, port 80 internal)
                       ...
                       ▼
                  timeful-backend   (Go/Gin, port 3002, not exposed to host)
                       │
                       │ mongodb://mongodb:27017
                       ▼
                  timeful-mongodb   (MongoDB 6.0, port 27017, not exposed to host)
```

<Note>
  In the separated-container production layout, the backend's port `3002` is declared with `expose` (container-internal only) rather than `ports` (host-mapped). Only the frontend container binds to `0.0.0.0:3002` on the host. The Go server detects the absence of `../frontend/dist` at startup and switches to API-only mode, returning a JSON 404 for any non-API route.
</Note>
...
<Accordion title="timeful-mongodb — MongoDB 6.0">
  - **Image:** `mongo:6.0`
  - **Internal port:** `27017` (not published to host in production)
  - **Database name:** `schej-it`
  - **Volumes:** `mongodb_data:/data/db`, `mongodb_config:/data/configdb`
  - **Health check:** `mongosh localhost:27017/test --quiet` with a 10-second interval and 5 retries
