---
title: "Deploy Timeful with Docker Compose"
sidebarTitle: "Docker"
...
Timeful ships as a three-container application — MongoDB, a Go backend, and an Nginx-served Vue.js frontend — all wired together with a single Docker Compose file. You can be running at `http://localhost:3002` in under five minutes using the pre-built images from GitHub Container Registry, or build everything locally if you need to customise the source.
...
<Tabs>
  ...
  <Tab title="Build from Source">
    Use this path when you want to modify the source code or test local changes before deploying.
...
<Note>
  The frontend container's Nginx proxy handles all `/api` and `/sockets/` traffic, so only a single port (`3002`) needs to be reachable from your browser. The backend container is intentionally not exposed to the host.
</Note>
...
| `timeful-backend` | `…/backend:latest` or built locally | `3002` | *(not exposed)* |
| `timeful-frontend` | `…/frontend:latest` or built locally | `80` | `3002` |

The frontend Nginx container proxies `/api` and `/sockets/` to the backend, serves the pre-built Vue.js bundle from `/usr/share/nginx/html`, and caches static assets for one year.
