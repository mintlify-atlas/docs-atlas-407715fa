---
title: "Availability Groups: Real-Time Team Scheduling"
...
When the `hasResponded` field is populated on the `Event`, it is derived by checking whether the currently authenticated user's ID is in the `Attendees` collection for that event.
