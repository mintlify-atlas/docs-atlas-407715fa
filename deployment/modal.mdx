---
title: "Host the Timeful Backend on Modal"
sidebarTitle: "Modal"
description: "Run the Timeful Go backend as a serverless long-running container on Modal, keeping one warm instance ready at all times."
---

[Modal](https://modal.com) is a serverless cloud platform that can run the Timeful Go backend as a persistent HTTP service. The `modal_backend.py` entrypoint in the repository packages the Go binary built from `Dockerfile.backend` and exposes it over HTTPS. Modal handles infrastructure, scaling, and cold-start behaviour — you only pay for actual compute time.
...
| `Dockerfile.backend` | Backend-only Docker image used by Modal for the build |
...
<Steps>
  ...
  <Step title="Deploy the Backend">
    ...
    Modal will:
    1. Build the Docker image using `Dockerfile.backend`.
